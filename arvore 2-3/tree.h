...
TreeNode *removeNo(TreeNode *arvore)
{
    ...
    // printf("removeNo");
    // no folha
    if (aux->father != NULL && aux->children[0] == NULL && aux->children[1] == NULL && aux->children[2] == NULL && aux->keys[0] == 0 && aux->keys[1] == 0 && aux->keys[2] == 0)
    {
        printf("\n\nfree");
    ...
    // no galho
    if (aux->keys[0] == 0 && aux->keys[1] == 0 && aux->keys[2] == 0)
    {
        printf("\n\ngalho");
        if (aux->children[0] != NULL)
        {
            aux->keys[0] = aux->children[0]->keys[0];
            aux->children[0]->keys[0] = 0;
            return removeNo(aux->children[0]);
        }

        else if (aux->children[1] != NULL)
        {
            aux->keys[1] = aux->children[1]->keys[0];
            aux->children[1]->keys[0] = 0;
            return removeNo(aux->children[1]);
        }

        else if (aux->children[2] != NULL)
        {
            aux->keys[2] = aux->children[2]->keys[0];
            aux->children[2]->keys[0] = 0;
            return removeNo(aux->children[2]);
        }
    }
